...
{
    public class DoubleRearrangement : IType
    {
        ...
        {
            var key = "";
            var matrix = GetMatrix(message, 2, message.Length - message.Length / 2);
            ...
            return new KeyValuePair<string, string>(key, GetString(matrix));
        }
        ...
        {
            var matrix = GetMatrix(message, message.Length - message.Length / 2, 2);
            int[] secondKey;
            var firstKey = GetKeys(key, out secondKey);
            var ouline = "";
